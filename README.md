...
- `Ctrl+Shift+Space` toggles recording.
...
- The global hotkey is not registered until the first model download succeeds.
...
- The tray icon is white when idle and red while recording, transcribing, or downloading the first model.
- Right-click the tray icon to exit, choose transcription language, or choose the model.
...
- The selected language and model are saved in `.local/settings/config.json`.
...
- `.local/settings/config.json` stores the selected language and model.
